...
# 词向量是具有空间意义的并不是简单的映射！
# 例如，我们希望单词 “love” 和 “adore” 这两个词在向量空间中是有一定的相关性的，
# 因为他们有类似的定义，他们都在类似的上下文中使用。
# 单词的向量表示也被称之为词嵌入。
# 
...
# 为了去得到这些词嵌入，我们使用一个非常厉害的模型 “Word2Vec”。
# 简单的说，这个模型根据上下文的语境来推断出每个词的词向量。
# 如果两个个词在上下文的语境中，可以被互相替换，那么这两个词的距离就非常近。
# 在自然语言中，上下文的语境对分析词语的意义是非常重要的。
# 比如，之前我们提到的 “adore” 和 “love” 这两个词，我们观察如下上下文的语境。
# 
# 
# ![caption](Images/SentimentAnalysis9.png)
# 
# 从句子中我们可以看到，这两个词通常在句子中是表现积极的，而且一般比名词或者名词组合要好。
# 这也说明了，这两个词可以被互相替换，他们的意思是非常相近的。
# 对于句子的语法结构分析，上下文语境也是非常重要的。
# 所有，这个模型的作用就是从一大堆句子（以 Wikipedia 为例）中为每个独一无二的单词进行建模，
...
# 根据句子的上下文来预测固定窗口中间那个词的向量。然后根据一个损失函数和优化方法，来对这个模型进行训练。
# 

# # Recurrent Neural Networks (RNNs)

# 现在，我们已经得到了神经网络的输入数据 —— 词向量，
# 接下来让我们看看需要构建的神经网络。NLP 数据的一个独特之处是它是时间序列数据。
# 每个单词的出现都依赖于它的前一个单词和后一个单词。
# 由于这种依赖的存在，我们使用循环神经网络来处理这种时间序列数据。
# 
# 循环神经网络的结构和你之前看到的那些前馈神经网络的结构可能有一些不一样。前馈神经网络由三部分组成，输入层，隐藏层和输出层。
# 
# ![caption](Images/SentimentAnalysis17.png)
# 
# 前馈神经网络和 RNN 之前的主要区别就是 RNN 考虑了时间的信息。
# 在 RNN 中，句子中的每个单词都被考虑上了时间步骤。
# 实际上，时间步长的数量将等于最大序列长度。
# 
# ![caption](Images/SentimentAnalysis18.png)
# 
# 与每个时间步骤相关联的中间状态也被作为一个新的组件，称为隐藏状态向量 h(t) 。
# 从抽象的角度来看，这个向量是用来封装和汇总前面时间步骤中所看到的所有信息。
...
# 长短期记忆网络单元，是另一个 RNN 中的模块。
# 从抽象的角度看，LSTM 保存了文本中长期的依赖信息。
# 正如我们前面所看到的，H 在传统的RNN网络中是非常简单的，
# 这种简单结构不能有效的将历史信息链接在一起。
# 举个例子，在问答领域中，假设我们得到如下一段文本，那么 LSTM 就可以很好的将历史信息进行记录学习。
# 
# 
# ![caption](Images/SentimentAnalysis4.png)
# 
# 在这里，我们看到中间的句子对被问的问题没有影响。
# 然而，第一句和第三句之间有很强的联系。
# 对于一个典型的RNN网络，隐藏状态向量对于第二句的存储信息量可能比第一句的信息量会大很多。
# 但是LSTM，基本上就会判断哪些信息是有用的，哪些是没用的，并且把有用的信息在 LSTM 中进行保存。
# 
...
# 
# 每个门都将 x(t) 和 h(t-1) 作为输入（没有在图中显示出来），
# 并且利用这些输入来计算一些中间状态。
# 每个中间状态都会被送入不同的管道，并且这些信息最终会汇集到 h(t) 。
# 为简单起见，我们不会去关心每一个门的具体推导。
# 这些门可以被认为是不同的模块，各有不同的功能。
# 输入门决定在每个输入上施加多少强调，遗忘门决定我们将丢弃什么信息，输出门根据中间状态来决定最终的 h(t) 。
# 
...
# 
# 作为该领域的一个最大玩家，Google 已经帮助我们在大规模数据集上训练出来了 Word2Vec 模型，
# 包括 1000 亿个不同的词！在这个模型中，谷歌能创建 300 万个词向量，每个向量维度为 300。
# 
# 在理想情况下，我们将使用这些向量来构建模型，但是因为这个单词向量矩阵相当大（3.6G），
# 我们用另外一个现成的小一些的，该矩阵由 GloVe 进行训练得到。
# 矩阵将包含 400000 个词向量，每个向量的维数为 50。
# 
# 我们将导入两个不同的数据结构，一个是包含 400000 个单词的 Python 列表，
# 一个是包含所有单词向量值得 400000*50 维的嵌入矩阵。
# 
...
positiveFiles = ['./training_data/positiveReviews/' + f for f in listdir('./training_data/positiveReviews/') if
                 isfile(join('./training_data/positiveReviews/', f))]
negativeFiles = ['./training_data/negativeReviews/' + f for f in listdir('./training_data/negativeReviews/') if
                 isfile(join('./training_data/negativeReviews/', f))]
numWords = []
...
# tf.nn.embedding_lookup() 函数来得到我们的词向量。
...
# 这是我们设置的一个超参数，我们需要对这个数值进行调试从而来找到最优的解。然后，我们会设置一个 dropout 参数，以此来避免一些过拟合。
# 
...
# 然而，当分析训练曲线的时候，我们应该注意到我们的模型可能在训练集上面已经过拟合了。
# 过拟合是机器学习中一个非常常见的问题，表示模型在训练集上面拟合的太好了，但是在测试集上面的泛化能力就会差很多。
# 也就是说，如果你在训练集上面取得了损失值是 0 的模型，但是这个结果也不一定是最好的结果。
# 当我们训练 LSTM 的时候，提前终止是一种常见的防止过拟合的方法。基本思路是，我们在训练集上面进行模型训练，同事不断的在测试集上面测量它的性能。
# 一旦测试误差停止下降了，或者误差开始增大了，那么我们就需要停止训练了。
# 因为这个迹象表明，我们网络的性能开始退化了。
# 
# 导入一个预训练的模型需要使用 TensorFlow 的另一个会话函数，称为 Server ，
# 然后利用这个会话函数来调用 restore 函数。这个函数包括两个参数，一个表示当前的会话，另一个表示保存的模型。
